...
export const ProjectConfigurator: React.FC<ProjectConfiguratorProps> = ({
  ...
  const instagramInquiryMessage = `Hi Abhishek, I generated a Project Configurator roadmap for my business "${businessName || niche || "New Project"}":
...
const handleInstagramInquiry = async () => {
  try {
    await navigator.clipboard.writeText(instagramInquiryMessage);
  } catch (error) {
    console.error("Failed to copy Instagram message:", error);
  }

  window.open(
    "https://www.instagram.com/abhishekkuntare/",
    "_blank",
    "noopener,noreferrer"
  );
};
